...
import (
	...
	v19paxspot "github.com/evmos/evmos/v18/app/upgrades/v19_paxspot"
	v20agent "github.com/evmos/evmos/v18/app/upgrades/v20agent"
	v21agent "github.com/evmos/evmos/v18/app/upgrades/v21agent"
	...
	"github.com/evmos/evmos/v18/x/attestor"
	...
	paxoracle "github.com/evmos/evmos/v18/x/paxoracle"
	...
	scheduler "github.com/evmos/evmos/v18/x/scheduler"
	...
	"github.com/evmos/evmos/v18/x/streams"
...
var (
	...
	ModuleBasics = module.NewBasicManager(
		...
		paxoracle.AppModuleBasic{},
		scheduler.AppModuleBasic{},
		// v21-agent-payments module basics. EIP712Helper precompile is
		// stateless (no module), so it does not appear here.
		streams.AppModuleBasic{},
		attestor.AppModuleBasic{},
	)
	...
	maccPerms = map[string][]string{
		...
		// v20-agent-foundations: scheduler module account holds escrowed
		// deposits between Schedule and Cancel/dispatch. No minter/burner
		// permissions — funds only enter via SendCoinsFromAccountToModule.
		schedulertypes.ModuleName: nil,
		// v21-agent-payments: streams module account holds escrowed
		// payment-stream caps (native PAX or ERC-20). Same custody model as
		// scheduler — recipient-only, no minter/burner. attestor has no
		// module account (stateless wrt funds — it only holds trusted root
		// certs in store).
		streamstypes.ModuleName: nil,
	}
)
...
type Evmos struct {
	*baseapp.BaseApp
	...
	// v21-agent-payments keepers. EIP712Helper precompile is stateless and
	// has no keeper field.
...
func NewEvmos(
	...
	app.PaxOracleKeeper = paxoraclekeeper.NewKeeper(
		keys[paxoracletypes.StoreKey],
		appCodec,
		authtypes.NewModuleAddress(govtypes.ModuleName),
		app.StakingKeeper,
	)

	// SchedulerKeeper is wired here so the EVM Scheduler precompile (0x0905,
	// v20-agent-foundations) can resolve its keeper at AvailablePrecompiles
	// time. The keeper depends on Erc20Keeper.CallEVMWithData via the
	// schedulertypes.EVMKeeper interface; passing app.Erc20Keeper satisfies it.
	app.SchedulerKeeper = schedulerkeeper.NewKeeper(
		keys[schedulertypes.StoreKey],
		appCodec,
		authtypes.NewModuleAddress(govtypes.ModuleName),
		app.AccountKeeper,
		app.BankKeeper,
	...
	// v21-agent-payments keepers. StreamsKeeper holds the open / settle /
	// close / updateRate state machine and pulls native PAX through
	// app.BankKeeper or ERC-20 through app.Erc20Keeper.CallEVM (see
	// x/streams/keeper/escrow.go). AttestorKeeper is gov-write / EVM-read
	// for trusted TEE root certificates and has no other-keeper deps.
	app.StreamsKeeper = streamskeeper.NewKeeper(
		keys[streamstypes.StoreKey],
		appCodec,
		authtypes.NewModuleAddress(govtypes.ModuleName),
		app.AccountKeeper,
		app.BankKeeper,
	...
	app.AttestorKeeper = attestorkeeper.NewKeeper(
		keys[attestortypes.StoreKey],
		appCodec,
		authtypes.NewModuleAddress(govtypes.ModuleName),
	)
	...
	app.EvmKeeper = app.EvmKeeper.SetHooks(
		evmkeeper.NewMultiEvmHooks(
			...
			// v20-agent-foundations: mirrors LaneRegistration /
			// LaneDeregistration events from the configured
			// AgentWalletFactory / ServiceRegistry contract into the
			// in-keeper LaneMember set. See x/feemarket/keeper/lane_hook.go.
	...
	*/
	...
	app.mm = module.NewManager(
		...
		paxoracle.NewAppModule(app.PaxOracleKeeper),
		scheduler.NewAppModule(app.SchedulerKeeper),
		// v21-agent-payments app modules. EIP712Helper precompile is
		// stateless and contributes no app module.
		streams.NewAppModule(app.StreamsKeeper),
		attestor.NewAppModule(app.AttestorKeeper),
	)
	...
	app.mm.SetOrderBeginBlockers(
		...
		paxoracletypes.ModuleName,
		schedulertypes.ModuleName,
		// v21-agent-payments — same after-paxoracle / before-consensusparam slot.
		streamstypes.ModuleName,
		attestortypes.ModuleName,
...
func (app *Evmos) BeginBlocker(ctx sdk.Context, req abci.RequestBeginBlock) abci.ResponseBeginBlock {
	// Paxeer never executes gov-driven software upgrades — every release ships
	// its full state contract in genesis (active_precompiles, module stores,
	// EIP-7702 wiring). To make the chain immune to a leftover MsgSoftwareUpgrade
	// plan from an old test run or accidental proposal, unconditionally clear
	// the upgrade plan at the start of every block. This neutralises both panics
	// in x/upgrade/abci.go:
	//   - "UPGRADE NEEDED at ..." (plan reaches height, no handler)
	//   - "BINARY UPDATED BEFORE TRIGGER" (handler exists, plan not yet ready)
	if _, hasPlan := app.UpgradeKeeper.GetUpgradePlan(ctx); hasPlan {
		app.UpgradeKeeper.ClearUpgradePlan(ctx)
	}

	// One-time PaxSpot precompile migration: remove old Stride/Osmosis outposts
	// (0x900, 0x901) and add PaxSpot precompiles (0x901-0x904).
	// This runs once — after 0x900 is removed, the check never fires again.
	app.migratePaxSpotPrecompilesOnce(ctx)
...
// migratePaxSpotPrecompilesOnce checks if the old Stride outpost (0x900) is still
// in ActivePrecompiles. If so, it removes old outposts and adds PaxSpot precompiles.
// This is idempotent — it only fires once per chain lifetime.
func (app *Evmos) migratePaxSpotPrecompilesOnce(ctx sdk.Context) {
	params := app.EvmKeeper.GetParams(ctx)

	oldStride := "0x0000000000000000000000000000000000000900"
	if !slices.Contains(params.ActivePrecompiles, oldStride) {
		return // already migrated
	}

	logger := ctx.Logger().With("module", "paxspot-migration")
	logger.Info("migrating EVM precompiles: removing old outposts, adding PaxSpot")

	removedOutposts := []string{
	...
	if err := app.EvmKeeper.SetParams(ctx, params); err != nil {
		logger.Error("failed to set EVM params after precompile migration", "error", err)
		return
	}
	logger.Info("PaxSpot precompile migration complete", "active_precompiles", len(filtered))
}
...
func (app *Evmos) setupUpgradeHandlers() {
	...
	// v19-paxspot upgrade handler
	app.UpgradeKeeper.SetUpgradeHandler(
		v19paxspot.UpgradeName,
		v19paxspot.CreateUpgradeHandler(
			app.mm, app.configurator,
			app.EvmKeeper,
		),
	)

	// v20-agent-foundations upgrade handler — activates Scheduler 0x0905,
	// sets the EIP-7702 activation block, locks the agent fee lane to disabled,
	// and runs x/scheduler module migrations.
	app.UpgradeKeeper.SetUpgradeHandler(
		v20agent.UpgradeName,
		v20agent.CreateUpgradeHandler(
			app.mm, app.configurator,
			app.EvmKeeper,
			&app.FeeMarketKeeper,
		),
	...
	// v21-agent-payments upgrade handler — activates PaymentStreams 0x0906,
	// TEEAttestor 0x0907, EIP712Helper 0x0908 and runs x/streams + x/attestor
	// module migrations. TEE root certs are NOT loaded here — gov ships them
	// post-upgrade via MsgUpdateTEERoots.
	app.UpgradeKeeper.SetUpgradeHandler(
		v21agent.UpgradeName,
		v21agent.CreateUpgradeHandler(
			app.mm, app.configurator,
			app.EvmKeeper,
		),
	)
	...
	case v17.UpgradeName, v18.UpgradeName, v181.UpgradeName:
		// no store upgrades
	case v19paxspot.UpgradeName:
		// add x/paxoracle module store
		storeUpgrades = &storetypes.StoreUpgrades{
			Added: []string{paxoracletypes.StoreKey},
		}
	case v20agent.UpgradeName:
		// add x/scheduler module store; precompile activation happens inside
		// the upgrade handler via EvmParams.ActivePrecompiles.
		storeUpgrades = &storetypes.StoreUpgrades{
			Added: []string{schedulertypes.StoreKey},
		}
	case v21agent.UpgradeName:
		// add x/streams and x/attestor module stores. The EIP712Helper
		// precompile is stateless, so no store key is added for it.
		// Precompile activation (0x0906, 0x0907, 0x0908) is performed in
		// the upgrade handler via EvmParams.ActivePrecompiles.
		storeUpgrades = &storetypes.StoreUpgrades{
			Added: []string{streamstypes.StoreKey, attestortypes.StoreKey},
		}
	}

	if storeUpgrades != nil && upgradeInfo.Height > 0 {
		// Normal path: on-chain upgrade plan exists, apply at scheduled height
