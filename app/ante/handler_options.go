...
func newEVMAnteHandler(options HandlerOptions) sdk.AnteHandler {
	return sdk.ChainAnteDecorators(
		...
		// v20-agent-foundations: gate EIP-7702 (SetCodeTx, type 0x04) on the
		// activation height + run structural auth-list validation. Placed
		// AFTER sigverify (so we know the outer-tx signature is valid) and
		// BEFORE account verification + gas consume (so a rejected pre-
		// activation 0x04 tx never pays gas).
		evmante.NewEthSetCodeTxDecorator(options.EvmKeeper),
