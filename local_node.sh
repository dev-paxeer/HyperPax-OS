...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# myKey address 0x7cb61d4117ae31a12e393a1cfa3bac666481d02e | pax10jmp6sgh4cc6zt3e8gw05wavvejgr5pwjnpcky
	VAL_KEY="mykey"
	VAL_MNEMONIC="gesture inject test cycle original hollow east ridge hen combine junk child bacon zero hope comfort vacuum milk pitch cage oppose unhappy lunar seat"

	# dev0 address 0xc6fe5d33615a1c52c08018c47e8bc53646a0e101 | pax1cml96vmptgw99syqrrz8az79xer2pcgp84pdun
	USER1_KEY="dev0"
	USER1_MNEMONIC="copper push brief egg scan entry inform record adjust fossil boss egg comic alien upon aspect dry avoid interest fury window hint race symptom"

	# dev1 address 0x963ebdf2e1f8db8707d05fc75bfeffba1b5bac17 | pax1jcltmuhplrdcwp7stlr4hlhlhgd4htqh3a79sq
	USER2_KEY="dev1"
	USER2_MNEMONIC="maximum display century economy unlock van census kite error heart snow filter midnight usage egg venture cash kick motor survey drastic edge muffin visual"

	# dev2 address 0x40a0cb1C63e026A81B55EE1308586E21eec1eFa9 | pax1gzsvk8rruqn2sx64acfsskrwy8hvrmafqkaze8
	USER3_KEY="dev2"
	...
	# dev3 address 0x498B5AeC5D439b733dC2F58AB489783A23FB26dA | pax1fx944mzagwdhx0wz7k9tfztc8g3lkfk6rrgv6l
	USER4_KEY="dev3"
	...
	# Set moniker and chain-id for Paxeer Network (Moniker can be anything, chain-id must be an integer)
	...
	jq '.app_state["crisis"]["constant_fee"]["denom"]="ahpx"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	jq '.app_state["inflation"]["params"]["mint_denom"]="ahpx"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	# Claim module account (pax prefix derived from same 20-byte module account)
	# 0xA61808Fe40fEb8B3433778BBC2ecECCAA47c8c47 || pax15cvq3ljql6utxseh0zau9m8ve2j8erz8m4cclz
	jq -r --arg amount_to_claim "$amount_to_claim" '.app_state["bank"]["balances"] += [{"address":"pax15cvq3ljql6utxseh0zau9m8ve2j8erz8m4cclz","coins":[{"denom":"ahpx", "amount":$amount_to_claim}]}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

	# Set base fee in genesis
	jq '.app_state["feemarket"]["params"]["base_fee"]="'${BASEFEE}'"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

	# Activate v20/v21 precompiles at genesis (skip the gov-upgrade dance)
	# 0x905 Scheduler (v20), 0x906 PaymentStreams (v21), 0x907 TEEAttestor (v21),
	# 0x908 EIP712Helper (v21). De-duped via 'unique'.
	jq '.app_state["evm"]["params"]["active_precompiles"] = (
		(.app_state["evm"]["params"]["active_precompiles"] // []) + [
			"0x0000000000000000000000000000000000000905",
			"0x0000000000000000000000000000000000000906",
			"0x0000000000000000000000000000000000000907",
			"0x0000000000000000000000000000000000000908"
		] | unique
	...
	evmosd add-genesis-account "$(evmosd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 100000000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	evmosd add-genesis-account "$(evmosd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000ahpx --keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	evmosd gentx "$VAL_KEY" 1000000000000000000000ahpx --gas-prices ${BASEFEE}ahpx --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
