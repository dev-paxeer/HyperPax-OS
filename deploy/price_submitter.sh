...
# ─── RPC failover pool ────────────────────────────────────────────────────────
# External Railway nodes ONLY — no local validator endpoints.
RPC_NODES=(
  "https://nodes-1-production-f6f1.up.railway.app"
  "https://nodes-2-production.up.railway.app"
  "https://nodes-3-production-98a7.up.railway.app"
  "https://nodes-4-production.up.railway.app"
  "https://nodes-5-production-27ee.up.railway.app"
  "https://nodes-6-production-5a34.up.railway.app"
  "https://nodes-7-production-78db.up.railway.app"
  "https://nodes-8-production-3c63.up.railway.app"
  "https://nodes-9-production-5a86.up.railway.app"
  "https://nodes-10-production-944d.up.railway.app"
  "https://nodes-11-production.up.railway.app"
  "https://nodes-12-production-b45a.up.railway.app"
  "https://nodes-13-production-7c6c.up.railway.app"
  "https://nodes-14-production-d0a0.up.railway.app"
  "https://nodes-15-production-8a1d.up.railway.app"
)
RPC_IDX=0
CURRENT_RPC="${RPC_NODES[0]}"
LOCAL_NONCE=-1

# Pick next RPC in round-robin, cycling on each call
next_rpc() {
  CURRENT_RPC="${RPC_NODES[$RPC_IDX]}"
  RPC_IDX=$(( (RPC_IDX + 1) % ${#RPC_NODES[@]} ))
}

# Fetch on-chain nonce from the first healthy node
refresh_nonce() {
  local addr="$1"
  local idx=0
  while [ $idx -lt ${#RPC_NODES[@]} ]; do
    local n
    n=$(cast nonce "$addr" --rpc-url "${RPC_NODES[$idx]}" 2>/dev/null) && {
      LOCAL_NONCE=$n
      return 0
    }
    idx=$((idx + 1))
  done
  log "ERROR: could not fetch nonce from any external node"
  return 1
}

# Send a cast transaction, cycling through RPCs until one succeeds or all fail.
# Uses explicit --nonce to avoid nonce desync across external nodes.
# Sets globals: TX_OUTPUT, RPC_USED, LOCAL_NONCE (incremented on success).
# NOT called inside $() — avoids subshell variable isolation.
rpc_send() {
  TX_OUTPUT=""
  RPC_USED=""
  local attempts=0
  ...
  while [ $attempts -lt $max ]; do
    next_rpc
    TX_OUTPUT=$(cast send "$@" --nonce "$LOCAL_NONCE" --rpc-url "$CURRENT_RPC" 2>&1) && {
      LOCAL_NONCE=$((LOCAL_NONCE + 1))
      RPC_USED="$CURRENT_RPC"
      return 0
    }
    log "  RPC FAIL ($CURRENT_RPC): $(echo "$TX_OUTPUT" | tail -1)"
    attempts=$((attempts + 1))
  done
  return 1
}
...
CHAINFLOW_STOCKS_API="https://chainflowtrading.com/api/v1/display"
CHAINFLOW_FUTURES_API="https://chainflowtrading.com/api/v1/futures"

# ─── Market definitions ───────────────────────────────────────────────────────
# Format: "PAIR_NAME|BINANCE_SYMBOL|COINGECKO_ID|KRAKEN_PAIR|CHAINFLOW_SYMBOL|CUSTOM_API_URL|CF_STOCK_SYM|CF_FUTURES_SYM"
# Empty field = source not available for this market.
# PAX and SID use custom Railway APIs exclusively.
# Stocks use ChainFlow /display API. Futures (NAS100, SPX) use ChainFlow /futures API.
...
log "  RPC pool:  ${#RPC_NODES[@]} external Railway nodes (no localhost)"
...
for market_def in "${MARKETS[@]}"; do
  IFS='|' read -r PAIR_NAME _ _ _ _ _ _ _ <<< "$market_def"
...
CHAINFLOW_SYMBOLS=""
for market_def in "${MARKETS[@]}"; do
  IFS='|' read -r _ _ _ _ CF_SYM _ _ _ <<< "$market_def"
...
# ─── Main loop ────────────────────────────────────────────────────────────────
while true; do
  # Refresh nonce at the start of each round to stay in sync
  if ! refresh_nonce "$VALIDATOR_ADDR"; then
    log "WARN: nonce refresh failed, sleeping and retrying"
    sleep "$INTERVAL"
    continue
  fi
  ...
  # Batch-fetch ChainFlow stocks
  CF_STOCKS_JSON=$(curl -sf --max-time 10 "${CHAINFLOW_STOCKS_API}?symbols=tsla,nvda,goog" 2>/dev/null) || {
    log "WARN: ChainFlow stocks fetch failed"
    CF_STOCKS_JSON=""
  }

  # Batch-fetch ChainFlow futures
  CF_FUTURES_JSON=$(curl -sf --max-time 10 "${CHAINFLOW_FUTURES_API}" 2>/dev/null) || {
    log "WARN: ChainFlow futures fetch failed"
    CF_FUTURES_JSON=""
  }

  for market_def in "${MARKETS[@]}"; do
    IFS='|' read -r PAIR_NAME BINANCE_SYM COINGECKO_ID KRAKEN_PAIR CF_SYM CUSTOM_URL CF_STOCK_SYM CF_FUTURES_SYM <<< "$market_def"
    ...
    # Source 6: ChainFlow Stocks (from cached batch response)
    if [ -n "$CF_STOCK_SYM" ] && [ -n "$CF_STOCKS_JSON" ]; then
      p=$(echo "$CF_STOCKS_JSON" | jq -r ".\"${CF_STOCK_SYM}\".price // empty" 2>/dev/null) || true
      if is_valid_price "$p"; then
        [ -n "$prices" ] && prices="${prices}"$'\n'"${p}" || prices="$p"
        source_count=$((source_count + 1))
        source_log="${source_log:+${source_log} }CFStock=$p"
      fi
    fi

    # Source 7: ChainFlow Futures (from cached batch response)
    if [ -n "$CF_FUTURES_SYM" ] && [ -n "$CF_FUTURES_JSON" ]; then
      p=$(echo "$CF_FUTURES_JSON" | jq -r ".\"${CF_FUTURES_SYM}\".price // empty" 2>/dev/null) || true
      if is_valid_price "$p"; then
        [ -n "$prices" ] && prices="${prices}"$'\n'"${p}" || prices="$p"
        source_count=$((source_count + 1))
        source_log="${source_log:+${source_log} }CFFutures=$p"
      fi
    fi
