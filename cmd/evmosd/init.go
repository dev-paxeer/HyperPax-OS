...
func InitCmd(mbm module.BasicManager, defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Set default seeds. Paxeer Network seeds are populated post-mainnet.
			// Operators should override via the --p2p.seeds flag or config.toml until
			// production seed addresses are published.
			seeds := []string{}
			...
			if chainID == "" {
				chainID = fmt.Sprintf("pax_9000-%v", tmrand.Str(6))
			}
