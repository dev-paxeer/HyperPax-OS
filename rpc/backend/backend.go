...
type Backend struct {
	...
	validatorCache   map[string]common.Address // ConsAddress -> EVM address cache
	validatorCacheMu sync.RWMutex
}

// WithRequestContext returns a shallow copy of the backend that uses the given context
// for downstream gRPC and Tendermint RPC calls. This enables request cancellation
// when the client disconnects.
func (b *Backend) WithRequestContext(ctx context.Context) *Backend {
	clone := *b
	clone.ctx = ctx
	return &clone
}
...
func NewBackend(
	...
	return &Backend{
		...
		indexer:             indexer,
		validatorCache:      make(map[string]common.Address),
	}
}
