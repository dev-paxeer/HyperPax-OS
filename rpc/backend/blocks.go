	...
	for _, event := range blockRes.EndBlockEvents {
		...
		for _, attr := range event.Attributes {
			if attr.Key == evmtypes.AttributeKeyEthereumBloom {
				return safeBytesToBloom(b.logger, []byte(attr.Value), blockRes.Height), nil
			}
		}
	}
...
// safeBytesToBloom converts a bloom event attribute value to ethtypes.Bloom
// without panicking on size mismatch. Cosmos SDK / CometBFT JSON-encode event
// attribute strings, which replaces invalid UTF-8 bytes with U+FFFD (3 bytes)
// so a bloom emitted as raw 256-byte binary may come back as 256 + 2*N bytes.
// Returns zero bloom on mismatch. Clients can still walk individual logs.
func safeBytesToBloom(logger log.Logger, val []byte, height int64) ethtypes.Bloom {
	if len(val) != ethtypes.BloomByteLength {
		logger.Debug(
			"block bloom attribute size mismatch (UTF-8 normalization in event indexing), returning zero bloom",
			"height", height,
			"got", len(val),
			"expected", ethtypes.BloomByteLength,
		)
		return ethtypes.Bloom{}
	}
	return ethtypes.BytesToBloom(val)
}
...
func (b *Backend) RPCBlockFromTendermintBlock(
	...
	consAddr := sdk.ConsAddress(block.Header.ProposerAddress).String()
	validatorAddr := b.getValidatorAddress(consAddr, block.Height)
...
// getValidatorAddress returns the EVM address for a consensus address, using the cache.
func (b *Backend) getValidatorAddress(consAddr string, height int64) common.Address {
	b.validatorCacheMu.RLock()
	cached, ok := b.validatorCache[consAddr]
	b.validatorCacheMu.RUnlock()
	if ok {
		return cached
	}
	...
	if err != nil {
		b.logger.Debug("failed to query validator operator address",
			"height", height, "cons-address", consAddr, "error", err.Error())
		return common.Address{}
	}

	accAddr, err := sdk.AccAddressFromBech32(res.AccountAddress)
	if err != nil {
		b.logger.Debug("failed to parse validator address", "error", err.Error())
		return common.Address{}
	}

	addr := common.BytesToAddress(accAddr)

	b.validatorCacheMu.Lock()
	b.validatorCache[consAddr] = addr
	b.validatorCacheMu.Unlock()
