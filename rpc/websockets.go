...
type websocketsServer struct {
	...
	api       *pubSubAPI
	logger    log.Logger
	rpcServer *rpc.Server // in-process RPC server for non-subscription calls
}

func NewWebsocketsServer(clientCtx client.Context, logger log.Logger, tmWSClient *rpcclient.WSClient, cfg *config.Config, rpcServer *rpc.Server) WebsocketsServer {
...
// tcpGetAndSendResponse routes JSON-RPC requests through the in-process RPC server
// instead of making an HTTP loopback call.
func (s *websocketsServer) tcpGetAndSendResponse(wsConn *wsConn, mb []byte) error {
	if s.rpcServer != nil {
		...
		req, err := http.NewRequestWithContext(context.Background(), "POST", "/", bytes.NewBuffer(mb))
		if err != nil {
			return errors.Wrap(err, "could not build in-process request")
		}
		req.Header.Set("Content-Type", "application/json")

		s.rpcServer.ServeHTTP(recorder, req)

		var wsSend interface{}
		if err := json.Unmarshal(recorder.body.Bytes(), &wsSend); err != nil {
			return errors.Wrap(err, "failed to unmarshal rpc response")
		}
		return wsConn.WriteJSON(wsSend)
	}

	// Fallback to HTTP loopback if no in-process server available
...
// inProcessResponseWriter implements http.ResponseWriter for in-process RPC calls.
	...
	go func() {
		...
		for {
			...
			case event, ok := <-headersCh:
				...
				bloom := bloomFromEndBlockEvents(data.ResultEndBlock.Events)

				header := types.EthHeaderFromTendermint(data.Header, bloom, baseFee)
...
// bloomFromEndBlockEvents extracts the block bloom from ABCI EndBlock events.
// Returns zero bloom on size mismatch (caused by UTF-8 normalization of the
// raw 256-byte binary attribute value during event indexing).
func bloomFromEndBlockEvents(events []abci.Event) ethtypes.Bloom {
