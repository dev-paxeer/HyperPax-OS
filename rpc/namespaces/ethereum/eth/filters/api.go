...
type Backend interface {
	...
	RPCFilterTimeout() time.Duration
}
...
func NewPublicAPI(logger log.Logger, clientCtx client.Context, tmWSClient *rpcclient.WSClient, backend Backend) *PublicFilterAPI {
	...
	filterTimeout := backend.RPCFilterTimeout()
	if filterTimeout <= 0 {
...
func (api *PublicFilterAPI) NewBlockFilter() rpc.ID {
	...
	api.filters[headerSub.ID()] = &filter{typ: filters.BlocksSubscription, deadline: time.NewTimer(api.deadline), hashes: []common.Hash{}, s: headerSub}
