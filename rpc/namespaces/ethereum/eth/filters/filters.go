...
func (f *Filter) Logs(_ context.Context, logLimit int, blockLimit int64) ([]*ethtypes.Log, error) {
	...
	if totalBlocks <= logsParallelism {
		// Sequential path for small ranges
	...
	sem := make(chan struct{}, logsParallelism)
	...
	for height := from; height <= to; height++ {
		...
		go func(h int64, i int) {
			...
			if err != nil {
				results[i] = blockResult{index: i, err: err}
				return
			}
			...
			filtered, err := f.blockLogs(blockRes, bloom)
			results[i] = blockResult{index: i, logs: filtered, err: err}
		}(height, idx)
	}
	wg.Wait()
	...
	for _, r := range results {
		...
		if len(logs)+len(r.logs) > logLimit {
			return nil, fmt.Errorf("query returned more than %d results", logLimit)
		}
