...
HTTPS_GIT := https://github.com/Paxeer-Network/hyperpax-os-cronosRelease.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=paxeer \
...
# abis-export copies every precompile ABI JSON to the SDK's `abis/` dir so the
# SDK codegen pipeline (paxeer-sdk) can regenerate TS + Python wrappers.
#
# Usage:
#   make abis-export                 # default DEST=../paxeer-sdk/abis
#   make abis-export DEST=/abs/path  # override destination
#
# Each ABI is copied as `<PrecompileName>.json` (PascalCase) to keep the SDK's
# import names consistent with the Solidity interfaces.
ABI_DEST ?= $(CURDIR)/../paxeer-sdk/abis

abis-export:
	@mkdir -p $(ABI_DEST)
	@echo "Exporting precompile ABIs to $(ABI_DEST)"
	@cp precompiles/paxspot/orob/abi.json      $(ABI_DEST)/OROBResolver.json
	@cp precompiles/paxspot/clearing/abi.json  $(ABI_DEST)/BatchClearing.json
	@cp precompiles/paxspot/oracle/abi.json    $(ABI_DEST)/OracleAggregator.json
	@cp precompiles/paxspot/pofq/abi.json      $(ABI_DEST)/PoFQScorer.json
	@cp precompiles/scheduler/abi.json         $(ABI_DEST)/Scheduler.json    2>/dev/null || true
	@cp precompiles/streams/abi.json           $(ABI_DEST)/PaymentStreams.json 2>/dev/null || true
	@cp precompiles/teeattestor/abi.json       $(ABI_DEST)/TEEAttestor.json    2>/dev/null || true
	@cp precompiles/eip712/abi.json            $(ABI_DEST)/EIP712Helper.json   2>/dev/null || true
	@echo "Done. Files at $(ABI_DEST):"
	@ls -1 $(ABI_DEST)

# abis-check verifies that each Go-side abi.json parses as valid go-ethereum ABI
# and that no method-name drift exists between the .go file and the .sol
# interface (smoke check; does not catch type drift).
#
# TODO(v20): wire this into CI under .github/workflows/build.yml. Until then,
# call it manually before tagging a release.
abis-check:
	@echo "Validating precompile ABI JSON parses..."
	@for f in precompiles/paxspot/*/abi.json precompiles/scheduler/abi.json precompiles/streams/abi.json precompiles/teeattestor/abi.json precompiles/eip712/abi.json; do \
		if [ -f "$$f" ]; then \
			python3 -c "import json,sys; json.load(open('$$f'))" || (echo "FAIL: $$f is not valid JSON" && exit 1); \
			echo "  ok: $$f"; \
		fi; \
	done

.PHONY: abis-export abis-check
