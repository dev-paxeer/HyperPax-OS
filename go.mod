...
replace (
	...
	// fix goroutine pool leak in memiavl snapshot writer (used by cronos/memiavl + versiondb)
	github.com/alitto/pond v1.8.3 => github.com/alitto/pond v1.9.1
	// use Cosmos-SDK fork to enable Ledger functionality
	github.com/cosmos/cosmos-sdk => github.com/evmos/cosmos-sdk v0.47.5-evmos.2
	// local iavl fork with sync.Mutex around lruCache to fix
	// `fatal error: concurrent map writes` from concurrent memiavl Tree.Get
	// callers (consensus DeliverTx + EVM JSON-RPC). Upstream cosmos/iavl
	// @ v0.21.0-alpha never synchronized cache/cache.go.
	github.com/cosmos/iavl => ./third_party/iavl
	// use Paxeer geth fork (local path during EIP-7702 backport dev loop;
	// switch to a remote v1.10.26-paxeer-7702-rcN tag once the patch is
	// reviewed, tagged on Paxeer-Network/go-ethereum, and ready to ship).
