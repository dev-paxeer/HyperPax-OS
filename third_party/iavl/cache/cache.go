...
type lruCache struct {
	// mu protects dict and ll. Every public method takes the lock because
	// even Get mutates the LRU list (MoveToFront). Without this, concurrent
	// callers (e.g. consensus DeliverTx + EVM JSON-RPC) reading the same
	// memiavl tree would race on the underlying Go map and crash the
...
// remove is the unlocked variant; callers must hold c.mu.
