...
func (k *Keeper) ApplyTransaction(ctx sdk.Context, tx *ethtypes.Transaction) (*types.MsgEthereumTxResponse, error) {
	...
	// EIP-7702 (SetCodeTx, type 0x04): pull the authorization list off the
	// transaction and stash it on the EVMConfig so ApplyMessageWithConfig
	// can apply delegation markers + nonce bumps against the same stateDB
	// the EVM frame will read. Gated by the runtime activation block — if
	// the chain has not crossed EIP7702BlockNumber the list is dropped and
	// the tx executes as a plain DynamicFeeTx-style call. Ante-side
	// gating (NewEthSetCodeTxDecorator) ensures pre-activation txs of
	// type 0x04 are rejected at admission, so this is defense-in-depth.
	if tx.Type() == ethtypes.SetCodeTxType && k.IsEIP7702Enabled(ctx) {
		if auths := tx.SetCodeAuthorizations(); len(auths) > 0 {
			cfg.SetCodeAuthList = auths
		}
	}
...
func (k *Keeper) ApplyMessageWithConfig(ctx sdk.Context,
	...
	// EIP-7702 authorization application. The list is only populated by
	// ApplyTransaction when the chain has crossed EIP7702BlockNumber AND
	// the tx is type 0x04. Per the Paxeer geth fork (Path B,
	// core/state_transition_setcode.go), ApplyAuthorizations:
	//   - charges PerAuthBaseCost / PerEmptyAccountCost per tuple
	//   - skips invalid tuples (signature, chain-id, nonce, existing code)
	//     without aborting the surrounding transaction
	//   - writes the 0xef0100||target delegation marker for valid tuples
	//   - bumps the authority nonce
	// Skip semantics are load-bearing — we MUST NOT abort the tx on per-
	// authorization failure (EIP-7702 §3).
	if len(cfg.SetCodeAuthList) > 0 {
		authGasUsed, _ := core.ApplyAuthorizations(stateDB, cfg.ChainConfig.ChainID, cfg.SetCodeAuthList)
		if leftoverGas < authGasUsed {
			return nil, errorsmod.Wrap(core.ErrIntrinsicGas, "eip-7702 authorization list exceeds gas budget")
		}
		leftoverGas -= authGasUsed
	}
