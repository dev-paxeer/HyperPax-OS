...
type TransactionArgs struct {
	...
	// v20-agent-foundations: EIP-7702 (SetCodeTx, type 0x04) authorization
	// list. Non-nil → ToTransaction emits a SetCodeTx. Field name matches
	// the EIP-7702 spec + viem / ethers.js wallet conventions.
	AuthorizationList []ethtypes.Authorization `json:"authorizationList,omitempty"`
}
...
func (args *TransactionArgs) ToTransaction() *MsgEthereumTx {
	...
	case args.AuthorizationList != nil:
		// v20-agent-foundations: EIP-7702 transaction type 0x04. Mirrors
		// the DynamicFee branch (1559 fee fields) plus the auth list.
		al := AccessList{}
		...
		setCode.AuthList = make([]SetCodeAuthorization, len(args.AuthorizationList))
		for i, a := range args.AuthorizationList {
			auth := SetCodeAuthorization{
				Address: a.Address.Hex(),
				Nonce:   a.Nonce,
			}
			if a.ChainID != nil {
				cid := sdkmath.NewIntFromBigInt(a.ChainID)
				auth.ChainID = &cid
			}
			if a.V != nil {
				auth.V = a.V.Bytes()
