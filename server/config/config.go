...
const (
	...
	// DefaultFilterTimeout is the default timeout for inactive filters
	DefaultFilterTimeout = 5 * time.Minute

	// DefaultBatchRequestLimit is the max number of requests in a JSON-RPC batch
	DefaultBatchRequestLimit = 100

	// DefaultBatchResponseMaxSize is the max cumulative response size for a JSON-RPC batch (10 MB)
	DefaultBatchResponseMaxSize = 10 * 1024 * 1024

	// DefaultLogsResponseMaxSize is the max response size for eth_getLogs (50 MB)
	DefaultLogsResponseMaxSize = 50 * 1024 * 1024

	// DefaultRPCProfile is the default RPC profile (empty = standard)
	DefaultRPCProfile = ""

	// DefaultAllowUnprotectedTxs value is false
	DefaultAllowUnprotectedTxs = false

	// DefaultMaxOpenConnections represents the amount of open connections
...
type JSONRPCConfig struct {
	...
	// FilterTimeout is the timeout duration for inactive filters before they are removed.
	FilterTimeout time.Duration `mapstructure:"filter-timeout"`
	// BatchRequestLimit is the maximum number of requests in a JSON-RPC batch.
	BatchRequestLimit int `mapstructure:"batch-request-limit"`
	// BatchResponseMaxSize is the maximum cumulative response size for a JSON-RPC batch (bytes).
	BatchResponseMaxSize int `mapstructure:"batch-response-max-size"`
	// LogsResponseMaxSize is the max response byte size for eth_getLogs (0 = unlimited).
	LogsResponseMaxSize int `mapstructure:"logs-response-max-size"`
	// RPCProfile configures the RPC node for a specific use case.
	// "indexer" loosens caps and enables expensive methods for indexers like Blockscout.
	// Empty string uses the standard profile.
	RPCProfile string `mapstructure:"rpc-profile"`
}
...
func DefaultJSONRPCConfig() *JSONRPCConfig {
	return &JSONRPCConfig{
		...
		LogsResponseMaxSize:      DefaultLogsResponseMaxSize,
		RPCProfile:               DefaultRPCProfile,
	}
}
...
// ApplyIndexerProfile adjusts JSON-RPC config for indexer-class usage
// when RPCProfile is set to "indexer". It loosens caps and enables all namespaces.
func (c *JSONRPCConfig) ApplyIndexerProfile() {
	if c.RPCProfile != "indexer" {
		return
	}
	c.API = GetAPINamespaces()
	c.LogsCap = 100000
	c.BlockRangeCap = 100000
	c.FeeHistoryCap = 1024
	c.FilterCap = 2000
	c.BatchRequestLimit = 500
	c.EVMTimeout = 30 * time.Second
	c.FilterTimeout = 30 * time.Minute
}
