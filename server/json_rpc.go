...
func StartJSONRPC(ctx *server.Context,
	...
	config.JSONRPC.ApplyIndexerProfile()
	...
	r := mux.NewRouter()
	var rpcHandler http.HandlerFunc
	if config.JSONRPC.BatchRequestLimit > 0 {
		rpcHandler = batchLimitHandler(rpcServer, config.JSONRPC.BatchRequestLimit)
	} else {
		rpcHandler = rpcServer.ServeHTTP
	}
	...
	tmWsClient = ConnectTmWS(tmRPCAddr, tmEndpoint, ctx.Logger)
	wsSrv := rpc.NewWebsocketsServer(clientCtx, ctx.Logger, tmWsClient, config, rpcServer)
	wsSrv.Start()
	return httpSrv, httpSrvDone, nil
}

// batchLimitHandler wraps an RPC server handler to reject batch requests exceeding the limit.
func batchLimitHandler(rpcServer *ethrpc.Server, limit int) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Quick check: if it starts with '[', it's a batch request
		...
		// Reconstruct the body for the RPC server
